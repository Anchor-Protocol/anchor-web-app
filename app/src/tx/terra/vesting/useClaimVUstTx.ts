...
export interface ClaimVUstTxParams {}

export function useClaimVUstTx() {
  ...
  const sendTx = useCallback(
    async (txParams: ClaimVUstTxParams, writer: TerraTxProgressWriter) => {
      const result = await terraSdk.vesting.claim(
      ...
      refetchQueries(ANCHOR_TX_KEY.VEST_CLAIM);
