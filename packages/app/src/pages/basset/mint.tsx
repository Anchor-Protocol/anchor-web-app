...
import { fabricateRegisterValidator } from '../../anchor-js/fabricators/register-validator'
...
const BassetMint: React.FunctionComponent<BassetMintProps> = () => {
  ...
  const [loading, error, whitelistedValidators = []] = useWhitelistedValidators()
  ...
  return ready(isReady, () => (
      <div className={style['basset-container']}>
        ...
        <article className={style.business}>
          ...
          {/* Validator selection */}
          <select value={validatorState} onChange={ev => setValidatorState(ev.currentTarget.value)}>
        ...
        <div>
          <input value={addressToWhitelist} onChange={ev => setAddressToWhitelist(ev.currentTarget.value)} />
          <ActionContainer render={execute => (
            <Button
              type={ButtonTypes.PRIMARY}
              transactional={true}
              onClick={() => execute(fabricateRegisterValidator({
                address: address,
                validatorAddress: addressToWhitelist,
              }))}
            >RegisterValidator</Button>
          )}
          />
        </div>
      </div>
