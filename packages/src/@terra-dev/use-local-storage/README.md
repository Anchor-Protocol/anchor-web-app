...
function Component() {
  const [value, setValue] = useLocalStorage('__storage_value__', () => 'default value')
...
function Component() {
  const [value, setValue] = useLocalStorageJson(
    '__storage_value__', 
    () => ({ value: 'default value' })
  )
  ...
  return (
    <div>
      ...
      <button onClick={() => setValue({ value: 'new value' })}>Update Value</button>
    </div>
  )
}
```